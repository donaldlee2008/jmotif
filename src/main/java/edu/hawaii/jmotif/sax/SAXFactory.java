...
public final class SAXFactory {
  ...
  public static DiscordRecords ts2Discords(double[] tsData, int windowSize, int alphabetSize)
    ...
    DiscordRecords discords = getDiscordsAlgorithm(tsData, windowSize, trie,
        DEFAULT_COLLECTION_SIZE, new LargeWindowAlgorithm());
   ...
   * @param windowSize PAA window size to use.
   * @param alphabetSize The SAX alphabet size.
   * @param dataAttributeName The WEKA attribute - essentially points on the instance attribute
   * which bears the data value in this case.
  ...
  public static DiscordRecords instances2Discords(Instances tsData, String dataAttributeName,
    ...
    DiscordRecords discords = getDiscordsAlgorithm(toRealSeries(tsData, dataAttribute), windowSize,
        trie, reportNum, new LargeWindowAlgorithm());
    Date end = new Date();
    ...
    while (idx < limit) {
      ...
      if (doRandomSearch) {
        ...
        while ((nextRandomVisitTarget = registry.getNextRandomUnvisitedPosition()) != -1) {
          ...
          double randomTargetDistance = EuclideanDistance.distance(currentLocations,
              randomTargetValues);
  ...
  /**
   * 
   * "We are given n, the length of the discords in advance, and we must choose two parameters, the
   * cardinality of the SAX alphabet size a, and the SAX word size w. We defer a discussion of how
   * to set these parameters until"
   * 
   * 
   * @param tsData timeseries.
