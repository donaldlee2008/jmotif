 ...
 * 
 */
public class UCRDigitsPatternsHunter2 extends UCRGenericClassifier {
  ...
  //
  private static final int[][] params = { { 190, 15, 5, NOREDUCTION } };
  ...
  public static void main(String[] args) throws IndexOutOfBoundsException, TSException, IOException {
    ...
    for (int[] p : params) {
      ...
      for (Entry<String, HashMap<String, Double>> e : tfidf.entrySet()) {
        ...
        for (int i = 0; i < MAX_SERIES_2PRINT; i++) {
          ...
          do {
            if (hits.get(k).length > 0) {
              ...
              System.out
                  .println(className + " " + Arrays.toString(trainData.get(className).get(k)));
              System.out.println(className
                  + " "
                  + Arrays.toString(seriesValuesAsHeat(trainData.get(className).get(k), className,
                      tfidf, WINDOW_SIZE, PAA_SIZE, ALPHABET_SIZE)));
