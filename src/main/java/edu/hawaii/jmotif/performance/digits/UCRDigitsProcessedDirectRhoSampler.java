 ...
 * 
 */
public class UCRDigitsProcessedDirectRhoSampler extends UCRGenericClassifier {
  ...
  // data
  //
  private static final String TRAINING_DATA = "data/digits/digits_processed_50-rho.csv";

  // output prefix
  //
  private static final String outputPrefix = "digits_direct_rho_processed";

  // SAX parameters to use
  //
  private static final int WINDOW_MIN = 20;
  private static final int WINDOW_MAX = 150;

  private static final int PAA_MIN = 5;
  private static final int PAA_MAX = 60;
  ...
  private static final int MAX_ITERATIONS = 26;

  private static List<String> globalResults = new ArrayList<String>();

  private UCRDigitsProcessedDirectRhoSampler() {
    super();
  }
...
//    Map<String, List<double[]>> trainData = resample(td, RESAMPLE_SIZE);
//    consoleLogger.fine("trainData classes: " + trainData.size() + ", series length: "
//        + trainData.entrySet().iterator().next().getValue().get(0).length);
//    for (Entry<String, List<double[]>> e : trainData.entrySet()) {
//      consoleLogger.fine(" training class: " + e.getKey() + " series: " + e.getValue().size());
