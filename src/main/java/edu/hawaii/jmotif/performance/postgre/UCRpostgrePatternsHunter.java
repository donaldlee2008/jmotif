package edu.hawaii.jmotif.performance.postgre;
 ...
 * 
 */
public class UCRpostgrePatternsHunter extends UCRGenericClassifier {
  ...
  //
  private static final int[][] params = { { 10, 7, 9, NOREDUCTION } };
  ...
  public static void main(String[] args) throws IndexOutOfBoundsException, TSException, IOException {
    ...
    Map<String, List<double[]>> testData = UCRUtils.readUCRData(TRAINING_DATA);
    ...
    for (int[] p : params) {
      ...
      int totalWords = tfidf.get("1").size();
      StringBuffer sb = new StringBuffer();
      for (Entry<String, HashMap<String, Double>> e : tfidf.entrySet()) {
        String className = e.getKey();
        int classEntries = countNonZero(e.getValue());
        sb.append(className).append(",").append(classEntries).append(",");
      }

      System.out.println(totalWords + "," + sb.toString());
  ...
  private static int countNonZero(HashMap<String, Double> hashMap) {
    ...
    for (Entry<String, Double> e : hashMap.entrySet()) {
      if (0.0D == e.getValue().doubleValue()) {
        res++;
      }
    }
    return res;
