...
dat = read.table("../data/wafer/wafer_TRAIN",header=F)
unique(dat[,1])
...
dm$index <- rep(1:152, 2)
dm$class <- c(rep("Class 1",(152*1)), rep("Class 2",(152*1)))
...
p = ggplot(dm, aes(x = index, y = value, group = X2, color=class)) +
  ...
  ggtitle("Sample of two classes from Wafer dataset") +
  scale_x_continuous("time ticks", limits=c(0,152), breaks=seq(0,152,25)) + 
  scale_y_continuous("Value",limits=c(-2,2.5),breaks=seq(-2,2.5,0.5))+
  theme(plot.title=element_text(size = 18, vjust = 2))
p
Cairo(width = 750, height = 250, file="wafer/all_classes.png", type="png", pointsize=12, 
...
Cairo(width = 750, height = 200, file="wafer/classes.png", type="png", pointsize=12, 
      bg = "transparent", canvas = "white", units = "px", dpi = "auto")
print(arrangeGrob(p1, p2, ncol=2))
dev.off()
