...
dat = read.table("../data/ChlorineConcentration/ChlorineConcentration_TRAIN",header=F)
unique(dat[,1])
...
dm$index <- rep(1:166, 3)
dm$class <- c(rep("Class 1",(166*1)), rep("Class 2",(166*1)), rep("Class 3",(166*1)))
...
p = ggplot(dm, aes(x = index, y = value, group = X2, color=class)) +
  ...
  ggtitle("Sample of three classes from ChlorineConcentration dataset") +
