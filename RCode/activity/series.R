...
#
full=(dat[grep("one*",dat[,1]),])[,-1]
plot=plotseries(full, c(1:5), "One hour events")
plot

#
two=(dat[grep("two*",dat[,1]),])[,-1]
#two=(dat[grep("1*",dat[,1]),])[,-1]
#
...
Cairo(width = 1200, height = 600, 
      file="activity/three_sets.png", 
...
library(ctc)
write.table(hc2Newick(hc), file="activity/euclidean.newick",row.names=FALSE,col.names=FALSE)
