...
plotseries=function(data, series, title){
  ...
  # collect series
  pre_series=znorm(data[series[1],])
...
#
#
dat = read.csv("activity/test-duration-series.csv",sep=" ",header=F)
#
one=(dat[grep("one*",dat[,1]),])[,-1]
plot=plotseries(one, c(1:5), "One hour events")
plot
for(i in 1:5){
  print(one[i,])
...
plot2=plotseries(two, c(1:5), "Two hour events")
plot2
for(i in 1:5){
  print(two[i,])
...
plot3=plotseries(three, c(1:5), "Three hour events")
plot3
for(i in 1:5){
  print(three[i,])
...
Cairo(width = 800, height = 400, 
      file="activity/test-duration-series.png", 
...
library(ctc)
write.table(hc2Newick(hc), file="activity/test-duration-HC-Euclidean.newick",row.names=FALSE,col.names=FALSE)
...
write.table(hc2Newick(hc), file="activity/test-duration-HC-dtw.newick",row.names=FALSE,col.names=FALSE)
...
write.table(hc2Newick(hc), file="activity/test-duration-HC-DWT.newick",row.names=FALSE,col.names=FALSE)
...
p=function(series, num, col){
  ...
  p1 = ggplot(dm, aes(x = index, y = value)) +
    theme_bw() + geom_line(colour=col,lwd=0.5)+
  ...
  #print(p1)
  #dev.off()
  Cairo(width = 120, height = 40, 
        file=paste("activity/duration/",fname,sep=""),
        type="pdf", pointsize=14, 
        bg = "transparent", canvas = "white", units = "px", dpi = 96)
  print(p1)
  dev.off()
}