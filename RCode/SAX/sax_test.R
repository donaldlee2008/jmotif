...
##
## last edited 04-03-2010, seninp@gmail.com
##
...
ts1.name <- "../data/timeseries03_5_4.csv"

## test data definitions
#
ts1.data <- t(c(1,1,3,5,8,7,6,2,3,4,3,1,4,6,9,8,5,2,3,4,2))
ts1.sax <- as.matrix(read.csv(file=ts1.name, header=FALSE, sep = ",", quote="\"", dec=".", fill = TRUE, comment.char=""))
...
#
plot(NULL, xlim=c(0,ts.length), ylim=c(ts.min,ts.max), main="trajectory1", xlab="ticks", ylab="Hrs")
...
#
plot(NULL, xlim=c(0,ts.length), ylim=c(-2,2), main="Z normalized trajectory1", xlab="ticks", ylab='')
