...
dat = read.table("../data/Two_Patterns/Two_Patterns_TRAIN",header=F)
unique(dat[,1])
range(dat[2,])
...
dm$index <- rep(1:128, 4)
dm$class <- c(rep("One",(128*1)), rep("Two",(128*1)), rep("Three",(128*1)),
              rep("Four",(128*1)))
...
p = ggplot(dm, aes(x = index, y = value, group = X2, color=class)) +
  ...
  ggtitle("Sample of four classes from Two Patterns dataset") +
...
p
Cairo(width = 750, height = 250, file="2patterns/all_classes.png", type="png", pointsize=12, 
...
p1 = ggplot(dm[dm$X2=="ones",], aes(x = index, y = value, group = X2, color="black")) +
  ...
  ggtitle("Class 1") +
  scale_x_continuous("time ticks", limits=c(0,128), breaks=seq(0,128,50)) + 
  scale_y_continuous("Value",limits=c(-2,3),breaks=seq(-2,3,0.5))+
  theme(plot.title=element_text(size = 18, vjust = 2))
p1
#
...
Cairo(width = 750, height = 500, file="2patterns/classes.png", type="png", pointsize=12, 
      bg = "transparent", canvas = "white", units = "px", dpi = "auto")
print(arrangeGrob(p1, p2, p3, p4, ncol=2))
dev.off()
