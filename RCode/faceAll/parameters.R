...
dat = read.table("../data/FaceAll/FaceAll_TRAIN",header=F)
unique(dat[,1])
range(dat[2,])
...
dm$index <- rep(1:131, 6)
dm$class <- c(rep("class 1",(131*1)), rep("class 4",(131*1)), rep("class 7",(131*1)),
              rep("class 9",(131*1)), rep("class 11",(131*1)), rep("class 13",(131*1)))
...
p = ggplot(dm, aes(x = index, y = value, group = X2, color=class)) +
  ...
  ggtitle("Sample of six classes from FaceAll data set") +
  scale_x_continuous("time ticks", limits=c(0,131), breaks=seq(0,131,25)) + 
  scale_y_continuous("Value",limits=c(-3,2.5),breaks=seq(-3,2.5,1))+
  theme(plot.title=element_text(size = 18, vjust = 2))
p
Cairo(width = 750, height = 250, file="faceAll/all_classes.png", type="png", pointsize=12, 
...
Cairo(width = 750, height = 500, file="faceAll/classes.png", type="png", pointsize=12, 
      bg = "transparent", canvas = "white", units = "px", dpi = "auto")
print(arrangeGrob(p1, p2, p3, p4, p5, p6, ncol=2))
dev.off()
