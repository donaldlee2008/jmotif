...
require(colorRamps)
#
dat = read.table(file="../direct/adiac_direct0.csv",sep=",")
names(dat)=c("Startegy","Window","PAA","Alphabet","Error")
paa_max=60
cbf.d.classic=dat[((dat[,1]=="CLASSIC")&(dat[,3]<paa_max)),]
cbf.d.exact=dat[((dat[,1]=="EXACT")&(dat[,3]<paa_max)),]
cbf.d.nored=dat[((dat[,1]=="NOREDUCTION")&(dat[,3]<paa_max)),]
...
t=100
cols=c(green2red(t), rep("red",(101-t)))
...
Cairo(width = 900, height = 350, file="jcool/adiac-direct.png", type="png", pointsize=12, 
...
print(cloud(PAA ~ Alphabet * Window, data = cbf.d.nored, col=cols[floor(cbf.d.nored$Error*100+1)],
            cex = .8, main = "Noreduction",
            screen = list(z = 10, x = -70, y = -20),
            par.settings = par.set,panel.aspect = 1.2,
            scales = list(arrows = FALSE),
            drape = TRUE, colorkey = FALSE,pretty=TRUE),
      split = c(1,1,3,1), mar = rep(0, 4), more = TRUE)
print(cloud(PAA ~ Alphabet * Window, data = cbf.d.exact, col=cols[floor(cbf.d.exact$Error*100+1)],
            cex = .8, main = "Exact",
            screen = list(z = 10, x = -70, y = -20),
            par.settings = par.set,panel.aspect = 1.2,
