...
dat = read.table("../data/Lighting7/Lighting7_TRAIN",header=F)
unique(dat[,1])
range(dat[2,])
...
fours=(dat[,dat[1,]==3])[-1,1]
fives=(dat[,dat[1,]==4])[-1,1]
sixs=(dat[,dat[1,]==5])[-1,2]
sevens=(dat[,dat[1,]==6])[-1,2]

# Take the twelve series and melt (or equivalently, stack) them:
dm <- melt(cbind(ones,twos,threes,fours,fives,sixs,sevens))
# add an index variable:
dm$index <- rep(1:319, 7)
dm$class <- c(rep("One",(319*1)), rep("Two",(319*1)), rep("Three",(319*1)),
              rep("Four",(319*1)), rep("Five",(319*1)), rep("Six",(319*1)), rep("Seven",(319*1)))
...
p = ggplot(dm, aes(x = index, y = value, group = X2, color=class)) +
  ...
  ggtitle("Sample of seven classes from Lightning7 dataset") +
  scale_x_continuous("time ticks", limits=c(0,319), breaks=seq(0,319,25)) + 
  scale_y_continuous("Value",limits=c(-2,12),breaks=seq(-2,12,2))+
  theme(plot.title=element_text(size = 18, vjust = 2))
p
Cairo(width = 750, height = 250, file="lightning7/all_classes.png", type="png", pointsize=12, 
...
p1 = ggplot(dm[dm$X2=="ones",], aes(x = index, y = value, group = X2, color="black")) +
  ...
  ggtitle("Class 1") +
  scale_x_continuous("time ticks", limits=c(0,319), breaks=seq(0,319,50)) + 
  scale_y_continuous("Value",limits=c(-2,12),breaks=seq(-2,12,3))+
  theme(plot.title=element_text(size = 18, vjust = 2))
p1
#
#
...
Cairo(width = 750, height = 600, file="lightning7/classes.png", type="png", pointsize=12, 
      bg = "transparent", canvas = "white", units = "px", dpi = "auto")
print(arrangeGrob(p1, p2, p3, p4, p5, p6, p7, ncol=2))
dev.off()
