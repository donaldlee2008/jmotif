...
p1=p1 + scale_color_manual(values=c("#619CFF","indianred2"), name = element_blank(), 
            ...
            guide = guide_legend(title.theme=element_text(size=0, angle=0),
                 keywidth = 0.8, keyheight = 1, label.theme=element_text(size=13, angle=0)))  +
...
# runtime plot
#
dat = read.csv("pkdd/time-error/cbf_performance6077_log2_nored.csv",header=F,skip=1)
...
# plot both together
#
grid.arrange(arrangeGrob(p1, p2, ncol=2, widths=c(0.8,1.2)))
#
...
print(arrangeGrob(p1, p2, ncol=2, clip=F))
dev.off()
