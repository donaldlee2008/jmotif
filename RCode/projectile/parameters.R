...
dat = read.csv("../data/shield/shield_train",sep=' ',header=F)
dat=t(dat)
unique(dat[1,])
...
dm$index <- rep(1:1223, 3)
dm$class <- c(rep("Class 0",(1223*1)), rep("Class 1",(1223*1)), rep("Class 2",(1223*1)))
...
p = ggplot(dm, aes(x = index, y = value, group = X2, color=class)) +
  ...
  scale_x_continuous("time ticks", limits=c(0,1223), breaks=seq(0,1223,200)) + 
  scale_y_continuous("Value",limits=c(-1.7,0.5),breaks=seq(-1.7,0.5,0.5))+
  theme(plot.title=element_text(size = 18, vjust = 2))
p

Cairo(width = 750, height = 250, file="shield/all_classes.png", type="png", pointsize=12, 
...
Cairo(width = 750, height = 350, file="shield/classes.png", type="png", pointsize=12, 
      bg = "transparent", canvas = "white", units = "px", dpi = "auto")
